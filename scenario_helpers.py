...
AGENT_FILES = ("Makefile", "filelist.f", "prompt.md", "rtl", "dv", "scripts")
_KEEP = {".hud"}  # the Workspace's ssh-credential dir, never wiped

# golden_pass self-test overlay: dropping the reference testbench in must score 1.0. Used by
# the no-agent validation driver (--validate golden_pass).
GOLDEN_OVERLAY = {
    "interrupt_contoller_cocotb_dv": (
        "solution/test_interrupt_controller.py",
        "dv/cocotb/test_interrupt_controller.py",
    ),
}
...
def _resolve_workspace_root() -> Path:
    ...
    root = Path(tempfile.gettempdir()) / "hud-chiprama-intc" / f"workdir-{os.getpid()}"
